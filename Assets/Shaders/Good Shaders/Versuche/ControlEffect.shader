		...
		SubShader{
			...
			// Upgrade NOTE: excluded shader from DX11; has structs without semantics (struct appdata members hack)
						...
						void disp(inout appdata v)
						{
							...
							bool verschiebung = false;
							//Get the center of the object
							float4 objectCenter = unity_ObjectToWorld[3];
							//objectCenter = objectCenter / 10;
						   ...
						   //dd = sqrt(dd);
							...
							if ((dd != 0.0) && (dd < radius)) {
							 ...
							 if (_Front == 1.0 && (dd != 0.0) && (dd <= radius) && centerVector.y < 0.0) {
								 ...
								 //v.vertex.y -= 0.1 * _Displacement;
								 //v.vertex.y += 0.001 * dist * _DistortionFactor;
								 //v.vertex.y += (1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055);
								 v.vertex.y += ((1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055)) * (2.18 / _Width);
								 //v.vertex.y += ((1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055)) * (0.218 / _Width);
							 }
							 if (_Back == 1.0 && (dd != 0.0) && (dd <= radius) && centerVector.y > 0.0) {
								 ...
								 //The scaling is oriented to the room width 2.18 (front room). This is why (2.18 / _Width) is necessary to make smaller rooms expand more and bigger rooms expand less
								 //(1 - dd / radius) ensures the dynamic and fluent enlargement of the room depending on the distance to the user
								 //v.vertex.y -= (1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055);
								 v.vertex.y -= (1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055) * (2.18 / _Width);
								 //v.vertex.y -= (1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055) * (0.218 / _Width);
							 ...
							 if (_Right == 1.0 && (dd != 0.0) && (dd <= radius) && centerVector.x > 0.0) {
								 ...
								 //v.vertex.x += 0.1 * _Displacement;
								 //v.vertex.x -= (1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055);
								 v.vertex.x -= (1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055) * (2.18 / _Width);
								 //v.vertex.x -= (1 - dd / radius) * (1 / dist * _DistortionFactor * 0.055) * (0.218 / _Width);
							 ...
							 if (centerVector.y > 0.0 && (dd != 0.0) && (dd <= radius)) {
					 ...
					 void surf(Input IN, inout SurfaceOutput o) {
						 ...
						 float4 objectCenterTest = unity_ObjectToWorld[3];
